...
{	
	 ...
	 */
	public static double innerProduct(JavaMatrix javaMatrix, JavaMatrix javaMatrix2) throws BadDimensionsException {
		if(javaMatrix.getSize() != javaMatrix2.getSize()) {
			throw new BadDimensionsException("Matrices should be the same dimension");
		}
		
		double innerProduct = 0;
		for (int i = 0; i < javaMatrix.getSize(); i++) {
			innerProduct += javaMatrix.get(i)*javaMatrix2.get(i);
		}
		return innerProduct;
	}
	...
	public JavaMatrix(int width, double[] vals, double[] vals2) {
		this.width = width;
	...
	public JavaVector getCol(int c) {
		this.transpose();
		JavaVector v = getRow(c);
		this.transpose();
		return v;
	}
	...
	public JavaVector getRow(int r) {
		double[] temp = Arrays.copyOfRange(matrix, width*r, width*(r+1));
		return new JavaVector(temp);
	}
	
	public JavaMatrix getRow(int start, int end) {
		double[] temp = Arrays.copyOfRange(matrix, width*start, width*end);
		return new JavaMatrix((end - start), temp);
	}
	 ...
	 * @throws BadDimensionsException 
	 */
	public void multiply(AbstractMatrix matrix) throws BadDimensionsException {
		//matrix1[m][n] matrix2[p][q]
	 ...
	 * @throws BadDimensionsException 
	 */
	public void multiply(JavaVector vector) throws BadDimensionsException {
